...
var additionQueueWorker = function(job, cb) {
  var oauth2Client = new OAuth2Client(config.googleId, config.googleSecret, job.serviceData.callbackUrl);
  oauth2Client.refreshToken_(job.serviceData.tokens.refresh_token, function(err, tokens) {
    uploadContact(job.task, job.anyfetchClient, tokens.refresh_token, cb);
  });
};
