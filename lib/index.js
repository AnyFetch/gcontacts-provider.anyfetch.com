...
var redirectToService = function(callbackUrl, cb) {
  googleapis.execute(function(err) {
    ...
    var oauth2Client = new OAuth2Client(config.google_id, config.google_secret, callbackUrl);
    ...
    cb(null, redirectUrl, {redirectUrl: redirectUrl});
  });
};

var retrieveTokens = function(reqParams, storedParams, cb) {
  var oauth2Client = new OAuth2Client(config.google_id, config.google_secret, storedParams.redirectUrl);
  ...
  oauth2Client.getToken(reqParams.code, function(err, tokens) {
    ...
    if(!tokens.refresh_token) {
      return cb(new Error("You're already subscribed to AnyFetch Google Contacts. To subscribe again, please revoke the permission from your Google Account using https://accounts.google.com/b/0/IssuedAuthSubTokens."));
    }
    ...
    cb(null, 'accountName', {refresh_token: tokens.refresh_token});
  });
};
...
var deletionQueueWorker = function(job, cb) {
  job.anyfetchClient.deleteDocument(job.task.url, cb);
};
